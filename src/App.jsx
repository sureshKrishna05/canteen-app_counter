...
function App() {
  ...
  // Only staff roles can access the counter app
  const isStaff = profile?.role === "canteen_admin"
    || profile?.role === "college_admin"
    || profile?.role === "owner";

  const ProtectedRoute = ({ children }) => {
    if (!currentUser) return <Navigate to="/login" replace />;
    if (currentUser && !isStaff) return <AccessDenied />;
    return children;
  };
...
const AccessDenied = () => (
  <div className="flex min-h-screen items-center justify-center flex-col gap-4">
    ...
    <button
      onClick={() => { import("./database/supabaseService").then(m => m.logout()); }}
